...
then
   ...
   python "$PYTHONSCRIPTS_PATH"/get_virtual_camera_images.py -trajectory ${TRAJ_NUM} -scenenetscripts_path "$SCENENET_SCRIPTS_PATH" -protobuf_path "$PROTOBUF_PATH" -dataset_path "$SCENENET_DATASET_PATH"/data/${DATASET_NAME}/ -linesfiles_path "$OUTPUTDATA_PATH"/${DATASET_NAME}_lines/ -output_path "$OUTPUTDATA_PATH"/${DATASET_NAME}/;
...
python "$PYTHONSCRIPTS_PATH"/split_dataset_with_labels_world.py -trajectory ${TRAJ_NUM} -protobuf_path "$PROTOBUF_PATH" -path_to_linesfiles "$OUTPUTDATA_PATH"/${DATASET_NAME}_lines/ -path_to_virtualcameraimages "$OUTPUTDATA_PATH"/${DATASET_NAME}/ -output_path "$OUTPUTDATA_PATH" -scenenetscripts_path "$SCENENET_SCRIPTS_PATH"
echo -e "\n**** Pickling files for trajectory ${TRAJ_NUM} in ${DATASET_NAME} set ****\n";
python "$PYTHONSCRIPTS_PATH"/pickle_files.py -splittingfiles_path "$OUTPUTDATA_PATH" -output_path "$PICKLEANDSPLIT_PATH"/${DATASET_NAME}/traj_${TRAJ_NUM}/ -dataset_name ${DATASET_NAME}

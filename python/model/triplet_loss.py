...
def _get_anchor_positive_triplet_mask(labels, max_dist=1.0):
    """Return a 2D mask where mask[a, p] is True iff a and p are distinct, have same instance
        label and are close in space.
    Args:
        labels: tf.float32 `Tensor` with shape [batch_size, 4]
        max_dist: max distance to be considered as close in space
    Returns:
    ...
    instance_equal = tf.equal(tf.expand_dims(
        instance_labels, 0), tf.expand_dims(instance_labels, 1))

    # Check if i and j are close in space
    ...
    mask = tf.logical_and(tf.logical_and(
        indices_not_equal, instance_equal), close_in_space)

    return mask


def _get_anchor_negative_triplet_mask(labels, max_dist=1.0):
    """Return a 2D mask where mask[a, n] is True if a and n have distinct labels or a and n are far away in space.
    Args:
        labels: tf.float32 `Tensor` with shape [batch_size, 4]
    Returns:
        mask: tf.bool `Tensor` with shape [batch_size, batch_size]
    ...
    mask = tf.logical_or(instance_not_equal, far_away_in_space)

    return mask


def _get_triplet_mask(labels, max_dist=1.0):
    ...
    # Check if labels[i, -1] == labels[j, -1]
    # Uses broadcasting where the 1st argument has shape (1, batch_size) and the 2nd (batch_size, 1)
    i_j_positive = _get_anchor_positive_triplet_mask(labels, max_dist=max_dist)
    i_k_negative = _get_anchor_negative_triplet_mask(labels, max_dist=max_dist)
